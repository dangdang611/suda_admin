<template>
  <div class="tableContainer">
    ...
    <div class="modify" v-show="isModify">
      <ModifyOrderMessage :edit="editMessage" />
    </div>
  </div>
</template>
...
import ModifyOrderMessage from "../components/ModifyOrderMessage.vue";
export default {
      ...
      showData: [
        {
          expressProduct: "速达特快",
          orderAddressFrom: [],
          orderAddressFromDetail: "",
          orderAddressTo: [],
          orderAddressToDetail: "",
          orderCompanyFrom: "",
          orderCompanyTo: "",
          orderFixedPhoneFrom: "",
          orderFixedPhoneTo: "",
  ...
  methods: {
    ...
    async search() {
      if (this.keys === "") {
        ...
        // 更新页面，展示搜索数据
        this.showData = await this.$api.order.searchOrder(this.keys, 1);
    ...
    async refreshData(size, currentPage) {
      ...
      // 为了省市区的回显，将大地址转化为数组
      // 如果这里直接对showData进行循环更改，那么会一直触发Modify的props更新，那么里面的监听事件会被疯狂触发
      this.showData.forEach((item) => {
        item.orderAddressFrom = item.orderAddressFrom.split(",");
        item.orderAddressTo = item.orderAddressTo.split(",");
      });
    ...
    async updateData(message) {
      ...
      // 注意点：await的表达式需返回一个promise的返回值否则无效
      // console.log(typeof message.orderAddressFrom);
