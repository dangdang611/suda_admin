<template>
  <div class="tableContainer">
    ...
    <div style="margin-bottom: 20px">
      ...
      <el-button @click="handleAdd()">新增包装服务</el-button>
    </div>
    <div class="content">
      <el-table
        :data="showData"
    ...
    <ItemPage :count="packageCount" v-show="isItemPage" />

    <div class="modify" v-show="isModify">
      <ModifyPackageMessage :edit="editMessage" />
    </div>
...
import ModifyPackageMessage from "../components/ModifyPackageMessage.vue";
...
export default {
  ...
  methods: {
    ...
    async search() {
      if (this.keys === "") {
        ...
        // 更新页面，展示搜索数据
        this.showData = await this.$api.packages.searchPackage(this.keys);
        // 隐藏分页器
    ...
    async handleDelete(index, row) {
      // 向数据库发送删除请求
      var data = await this.$api.packages.delPackage(row.packageId);
      // console.log(data);
    ...
    async refreshData(size, currentPage) {
      ...
      // 更新展示数据
      this.showData = await this.$api.packages.getPackage(
        this.currentPage,
        this.size
      );
      // 更新分页器
    ...
    async updateData(message) {
      ...
      var data = await this.$api.packages.updatePackage(message);
      this.tips(data.code, data.message);
    ...
    async delSelection() {
      if (this.multipleSelection.length == 0) {
        ...
        // 向数据库发送删除请求
        var data = await this.$api.packages.delPackages(packageIds);
        this.tips(data.code, data.message);
    ...
    async getPackageCount() {
      ...
      this.packageCount = await this.$api.packages.getPackageCount();
    },
    closeModify() {
      this.isModify = false;
    },
    ...
    async addPackage(packages) {
      var data = await this.$api.packages.addPackage(packages);
      this.tips(data.code, data.message);
  ...
  mounted() {
    this.$bus.$on("refreshPackageTable", this.refreshData);
    this.$bus.$on("closeModify", this.closeModify);
    this.$bus.$on("updatePackageTable", this.updateData);
    this.$bus.$on("AddPackageTable", this.addPackage);
    this.$bus.$on("closeAdd", this.closeAdd);
    // 手动调用
    this.refreshData(3, 1);
  },
  destroyed() {
    this.$bus.$off("refreshPackageTable");
    this.$bus.$off("closeModify");
    this.$bus.$off("updatePackageTable");
    this.$bus.$off("AddPackageTable");
